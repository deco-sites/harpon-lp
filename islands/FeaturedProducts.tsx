         ...
         <div class='container p-4 mb-[150px] xs:mt-0 xs:ml-0 xs:mb-[150px] 1xs:mt-0 1xs:-ml-[15px] md:h-[550px]'>
            ...
            {loading ? (
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 ml-[20px] -mb-[150px] lg:gap-2 lg:mt-4 md:grid-cols-4 gap-5 mt-4 xs:grid-cols-2 xs:gap-[30px] 1xs:grid-cols-2 1xs:gap-5 xl:grid 2xl:grid">
                        ...
                        {categories.map(category => (
                            <a href='/productlist' onClick={() => handleCategoryClick(category.name)}>
                                <div key={category.id} class="group section border border-solid border-[#E6E6E6] relative transition duration-300 ease-in-out hover:bg-[#E9F408] xs:w-[150px] xs:h-[150px] 1xs:w-[180px] 1xs:h-[200px] md:w-[180px] md:h-[250px]">
                                    <p class='flex justify-center my-[30px] font-bold text-lg uppercase text-center font-[Albert Sans] group xs:mt-[50px] xs:ml-0 xs:pt-[10px] xs:text-sm  1xs:text-base 1xs:ml-[50px] 1xs:pt-[30px] md:pl-5 md:pt-[90px]'>{category.name}</p>
                                </div>
                            </a>
                        ))}
                    </div>
