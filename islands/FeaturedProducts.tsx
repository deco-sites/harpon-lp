...
export const FeaturedProducts: FunctionalComponent = () => {
    ...
    return (
        <>
            <div>
                <a href="/promocao-carbide" target='_blank'><img class='w-full -mt-[70px] xs:-mt-[70px] 1xs:-mt-[70px] 2xl:-mt-[110px]' src='banner-atual.png' alt='Banner Cabide' ></img></a>
            </div>
         <div class='container p-4 mb-[150px] xs:mt-0 xs:ml-0 xs:mb-[150px] 1xs:mt-0 1xs:-ml-[15px] md:h-[550px] lg:mt-[142px]'>
            <h1 class='text-[36px] ml-[20px] font-normal xs:text-[20px] 1xs:text-2xl md:ml-0'>
            ...
            {loading ? (
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 ml-[20px] -mb-[150px] lg:gap-2 lg:mt-4 md:ml-0 md:grid-cols-4 gap-5 mt-4 xs:grid-cols-2 xs:gap-[30px] 1xs:grid-cols-2 1xs:gap-5 xl:grid 2xl:grid">
                        {/* Renderiza as categorias */}
                        {categories.map(category => (
                            <a href='/productlist' onClick={() => handleCategoryClick(category.name)}>
                                <div key={category.id} class="group section border border-solid border-[#E6E6E6] relative transition duration-300 ease-in-out hover:bg-[#E9F408] xs:w-[150px] xs:h-[150px] 1xs:w-[180px] 1xs:h-[200px] md:w-[180px] md:h-[180px]">
                                    <p class='flex justify-center my-[30px] font-bold text-lg uppercase text-center font-[Albert Sans] group xs:mt-[50px] xs:ml-0 xs:pt-[10px] xs:text-sm  1xs:text-lg 1xs:ml-0 1xs:mt-[50px] 1xs:pt-[30px] md:pl-0 md:pt-[50px]'>{category.name}</p>
                                </div>
                            </a>
                        ))}
                    </div>
